    ...
    <ModalShell label={`Share ${file.original_name}`} shellClassName="w-full max-w-sm" onClose={onClose}>
      {(close) => (
        <div className="bezel-core space-y-3 p-4">
          <div className="truncate font-serif text-lg italic">{shareUrl ? "Share link created" : "Share file"}</div>
          {!shareUrl ? (
            <>
              <p className="truncate font-mono text-[11px] text-mocha-muted">{file.original_name}</p>
              <div>
              ...
              {error && <div className="rounded-2xl border border-red-400/20 bg-red-400/10 px-4 py-2.5 font-mono text-xs text-red-200">{error}</div>}
            ...
            <>
              <div className="break-all rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 font-mono text-xs">{shareUrl}</div>
              <div className="flex gap-2">
